...
docker exec -ti \
    $DOCKER_CONTAINER_ID /bin/bash -xec "bash -xe ci-source/build.sh; rm -rf ci-source/build; mkdir ci-source/build; git config --global --add safe.directory /ci-source; cd ci-source/build; cmake -DCMAKE_BUILD_TYPE=Release ..; make $BUILD_FLAGS; chmod -R a+rw ../build;"
