...
docker exec -ti \
    $DOCKER_CONTAINER_ID /bin/bash -xec "bash -xe ci-source/build.sh; rm -rf ci-source/build; mkdir ci-source/build; cd ci-source/build; cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo ..; make $BUILD_FLAGS; chmod -R a+rw ../build;"
